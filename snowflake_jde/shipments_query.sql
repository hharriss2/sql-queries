...
SELECT
    ...
    ,shipto_namemailing as customer_name

    -- item data
    ,sh.litm_identifier2nditem as item_number
    ,sh.dsc1_descriptionline1 as item_desc1
    ,sh.dsc2_descriptionline2 as item_desc2
    ,im.itm_identifiershortitem as item_identifier
    ,im.supergroup_desc1 as item_category
    ,im.ultragroup_desc1 as item_grouping
    ,im.srp7_salesreportingcode7 as brand_code
    ,im.srp7_salesreportingcode7_desc1 as brand_desc
...
FROM djus_jde.public.ODS_F42119_SALES_HISTORY SH -- sales order history, stuff that has gone to GL (has gl date) or been canceled (has no gl date)
         JOIN djus_jde.public.ODS_F03012_CUSTOMER_MASTER_LOB CM
              ON CM.CO_COMPANY = SH.KCOO_COMPANYKEYORDERNO 
              AND CM.AN8_ADDRESSNUMBER = SH.AN8_ADDRESSNUMBER
         JOIN djus_jde.public.ODS_F0101_ADDRESS_BOOK_MASTER ABBT -- Address Book Table Joined on the BillTo account number
              ON ABBT.AN8_ADDRESSNUMBER = SH.AN8_ADDRESSNUMBER
         JOIN djus_jde.public.ODS_F0101_ADDRESS_BOOK_MASTER ABST -- Address Book Table Joined on the ShipTo account number
              ON ABST.AN8_ADDRESSNUMBER = SH.SHAN_ADDRESSNUMBERSHIPTO
         LEFT JOIN djus_jde.public.ODS_F4101_ITEM_MASTER IM -- Item Master Table for anything you need out of there
                   ON IM.ITM_IDENTIFIERSHORTITEM = SH.ITM_IDENTIFIERSHORTITEM
         LEFT JOIN djus_jde.PUBLIC.ODS_MSD_ITEM_COMPLETE IM2 -- Curated Item Master Table for better reporting
                   ON IM2.ITEMID = SH.ITM_IDENTIFIERSHORTITEM
        left join djus_jde.public.ods_f0006_business_unit_master bu
...
AND SH.DCTO_OrderType NOT IN (SELECT ORDER_TYPE FROM djus_jde.PUBLIC.JDE_SALES_ORDER_TYPE_EXCLUSIONS)
AND SH.LNTY_LineType IN (SELECT LINE_TYPE FROM djus_jde.PUBLIC.JDE_SALES_LINE_TYPE_INCLUSIONS)
...
and sh.lnty_linetype in ('D', 'S', 'S7')
)
,fix_retailers as 
...
select item_number as model
    ...
    ,'Dorel Home Furniture' as division
    ,retailer
    ,order_id
    ,warehouse_code
from fix_retailers
