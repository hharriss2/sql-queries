 SELECT t1.group_id AS "Group ID",
           ...
           FROM ( SELECT l4_units_sold.group_id,
                   ...
                   FROM ( SELECT g.group_id,
                          ...
                          WHERE (rs.wm_week IN ( SELECT DISTINCT retail_sales.wm_week
                                  ...
                                  WHERE retail_sales.wm_week <> (( SELECT max(retail_sales_1.wm_week) AS maxa
   ...
   FROM pos_reporting.retail_sales retail_sales_1))
                          ...
                          GROUP BY g.group_id, g.collection_name) l52_units_sold
          ...
          WHERE 1 = 1 AND ((t1_1.group_id IN ( SELECT group_ids.group_id
                   FROM group_ids
                  WHERE (group_ids.tool_id IN ( SELECT item_class.tool_id::integer AS tool_id
                           FROM item_class
                          WHERE item_class.class = 'A'::text)))) OR cbm.cat = 'Mattresses'::text) AND ((t1_1.l4_units - t2.l52_units) / t2.l52_units) >= 0.30) t1;