...
--	                        else w.upc end as upc
	...
	--                        				, item_num
	                        ...
	                        left join 
	                        		(--start s
	/*Ships logic takes into account most recent model, upc, and tool ID dates. 
...
--count repeats query above and finds duplicates so we can filter these values out. 
,count_pr as (
...
--	              , w.item_id
            ,case 
			when model_tool.tool_id = '' then w.item_id 
			when model_tool.tool_id::text = '\N'::text then w.item_id--used to omit duplicate base id's that are \N
             when model_tool.tool_id is not null then model_tool.tool_id 
             else w.item_id end  as item_id
