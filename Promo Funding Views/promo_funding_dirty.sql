...
/*start promo dirty view 2*/
create view misc_views.promo_funding_dirty2 as (
SELECT DISTINCT pf.id,
    cbm.cat,
    coalesce(s.model, pf.model) as model,
    pr.product_name,
  ...
  WHERE 1 = 1 AND NOT 
  					(cbm.cat IN ( 
  							SELECT DISTINCT cat_by_model.cat
           					FROM cat_by_model
           						)
           			) 
          		...
          		OR NOT (pf.tool_id IN ( 
          					SELECT DISTINCT tool_id_view.tool_id
           					FROM tool_id_view)) 
           		OR s.model::text = '#N/A'::text
           		 OR pf.tool_id = '#N/A'::text 
           		 OR cbm.cat = '#N/A'::text
           		  OR pf.tool_id = ''::text
/*end promo dirty view 2*/
);
