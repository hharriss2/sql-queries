 SELECT t1.tool_id AS "Tool ID",
   ...
   FROM pos_reporting.retail_sales retail_sales_1))
          ...
          WHERE 1 = 1 AND ((t1_1.tool_id IN ( SELECT item_class.tool_id::integer AS tool_id
                   FROM item_class
                  WHERE item_class.class = 'A'::text)) OR cbm.cat = 'Mattresses'::text) AND ((t1_1.l4_units - t2.l52_units) / t2.l52_units) >= 0.30 AND model_tool.model IS NOT NULL) t1;