--view for target shipment numbers
create or replace view walmart.core.ships_target as 
(
with dimi1 as 
(
select item_sk
    ,retailer_upc
    ,split_part(retailer_item_url,'A-',2) as item_id
    ,retailer_item_url
    ,row_number() over ( partition by item_sk order by retailer_item_url desc )  as item_seq
from dorel_dwh.edw.xref_item_retailer
where 1=1
and retailer_item_url like '%target.com%'
...
where retailer_name in('Target.com','Target Plus')
and retailer_type = 'Ecommerce'
and lower(customer_name) not like '%juv%'
)
,ni as  -- new item
(
select item_sk
