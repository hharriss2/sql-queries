--shipment view for amazon 
create or replace view walmart.core.ships_amazon as 
(
with dimi1 as 
(
select item_sk
    ,retailer_sku
    ,replace(retailer_item_url,'?psc=1','') as retailer_item_url_1
    ,replace(retailer_item_url_1,'/','') as retailer_item_url
    ,row_number() over ( partition by item_sk order by retailer_item_url )  as item_seq
from dorel_dwh.edw.xref_item_retailer
where 1=1
and retailer_sku like 'B0%'
)
,di as 
(
select
    item_sk
    ,coalesce(retailer_sku, right(retailer_item_url,10)) as item_id
    ,retailer_item_url
from dimi1
where item_seq =1
)
...
where retailer_name like '%Amazon%'
and retailer_name not in ('Amazon.uk','Amazon.ca','Amazon Mexico','Amazon.ca Direct')
)
,ni as  -- new item
(
select item_sk
